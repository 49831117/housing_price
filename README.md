...
- 數個鄉鎮市區共用相同郵遞區號，raw data 的欄位亦無區分，
   ...
   如：嘉義市不論東區或西區，raw data 的資料皆為「嘉義市」，

   若未將其區分，繪得的圖會如第一次測試一樣，嘉義市部分沒有資料。
    - 地址與地段前處理：
...
- 資料分析：
   1. 因聚焦在各鄉鎮市區建物相關交易中每坪單價數據統計，故在有效的 `392,158` 筆資料中，
   
      篩選出「建物」、「房地(土地+建物)」、「房地(土地+建物)+車位」三種交易標的，共計 `188,325` 筆。
### 表格與視覺化
